...
import { triggerAnalysis } from '@/lib/services/userService';

export const useAnalysisTrigger = (fnrkUserId, currentPage = null, refetchFunction = null) => {
  ...
  const handleTriggerAnalysis = async () => {
    ...
    try {
      await triggerAnalysis(fnrkUserId);

      // Show success toast
      const message =
        currentPage === 'journey'
          ? 'Analysis triggered successfully. Redirecting to accounts page...'
          : currentPage === 'accounts'
            ? 'Analysis triggered successfully. Refreshing page...'
            : 'Analysis triggered successfully. Redirecting...';
      ...
      actions.setNotification({
        type: 'error',
        message: `Failed to trigger analysis: ${error.message || 'Unknown error'}`,
        duration: 5000
      });
    } finally {
